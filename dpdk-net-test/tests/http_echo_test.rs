...
/// Run the HTTP server: accept connections and serve HTTP/1.1
async fn run_http_server<F: Future<Output = ()>>(mut listener: TcpListener, shutdown: F) {
    println!("HTTP Server: listening on {}:{}", SERVER_IP, SERVER_PORT);
                ...
                match result {
                    Ok(stream) => {
                        ...
                        tokio::task::spawn_local(async move {
                            ...
                            match result {
                                Ok(()) => println!("HTTP Server {}: connection closed", id),
                                Err(e) => eprintln!("HTTP Server {}: error: {}", id, e),
                            }
                        });
                    }
                    Err(e) => {
                        eprintln!("HTTP Server: accept failed: {:?}", e);
                        break;
                    }
                }
            }
...
async fn run_http_test(
    ...
    // Spawn HTTP server
    let server_handle = tokio::task::spawn_local(run_http_server(listener, async {
