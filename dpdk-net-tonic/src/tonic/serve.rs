...
//!
//! Thin wrapper over [`crate::axum::serve`] that accepts tonic's [`Routes`]
...
/// `.into_axum_router()` and delegates to [`crate::axum::serve`].
///
...
pub async fn serve(listener: TcpListener, routes: Routes, shutdown: impl Future<Output = ()>) {
    ...
    crate::axum::serve(listener, axum_router, shutdown).await;
}
